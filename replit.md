...
- Pre-pilot demo cleanup: api-server uses AWS_DATABASE_URL (RDS) when set, else DATABASE_URL. Demo accounts standardized on `demo%@itsviva.com` (Dr. Riley Kim is `demo+riley@itsviva.com`, dev-login + seeded patients use `demo+...@itsviva.com`). Operator analytics endpoints (`/api/internal/metrics`, `/analytics/summary`, `/care-loop/summary`, `/care-loop/trend`, `/analytics/usage`, `/treatment/*`, drilldowns) exclude demo users via the helper at `artifacts/api-server/src/lib/demoFilter.ts` (DEMO_EMAIL_LIKE constant, `demoUserIdsSelect()` drizzle subquery, `excludeDemoCol(qualifiedCol)` raw-SQL fragment). Demo-doctor invites in `routes/patients.ts` rewrite the placeholder email to `demo+inv-{phone}-{hex}@itsviva.com` so newly-invited demo patients propagate the demo flag. Backups (CSV per table, gitignored): `backups/aws_pre_wipe_<ts>/`. The seed `pnpm --filter @workspace/scripts run seed:pilot` wipes all `demo%@itsviva.com` users + cascades and reseeds 10 demo doctors + 100 demo patients.
