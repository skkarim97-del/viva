...
- **Completion Tracking**: Daily completion rate + weekly consistency score ("Consistency: 80% this week"). Consistency card shown only when history exists. Data stored per-day with per-action category + recommended/chosen breakdown.
...
- **Refine Categories**: 4 categories × 4 options each. Energy (Excellent/High/Medium/Low), Stress (Low/Moderate/High/Very High), Hydration (Well Hydrated/Good/Low/Dehydrated), Training (None/Light/Moderate/Intense). Stacked layout (label above, chips below, flex: 1 equal width).
