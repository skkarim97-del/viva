...
- **Progressive disclosure**: Only feeling input visible by default. Energy, Stress, Hydration, Life Load, and Training hidden behind "Refine your day" card toggle. Reduces visual clutter.
- **State-based color system**: Refine chips use semantic colors when selected. Energy: high=green, medium=gray, low=red. Stress: low=green, moderate=amber, high=red. Hydration: good=blue, low=amber. Life Load: light=green, normal=gray, busy=amber, overwhelmed=red. Training: none=gray, light=light-blue, training=blue. Selected state = tinted background (18% opacity) + colored border (40% opacity) + colored text. Unselected = neutral.
...
- **Daily Focus**: Short motivational focus line displayed in its own card above Your Day (e.g., "Focus on consistency today", "Maximize today").
- **Your Day (Actionable Plan)**: 4-5 checkable actions per day. Each has: circular checkbox, category icon, label (MOVE/FUEL/RECOVER/MIND/HYDRATE), short actionable text. Tap to complete → green fill + checkmark + strikethrough + subtle green background. Progress counter (e.g., "2/4"). Actions persisted via `@viva_completions` in AsyncStorage as CompletionRecord array.
- **Completion Tracking**: Daily completion rate + weekly consistency score ("Consistency: 80% this week"). Consistency card shown only when history exists. Data stored per-day with per-action category breakdown.
...
- **Wellness Inputs**: Feeling (Great/Good/Tired/Exhausted/Stressed) always visible in its own card. Energy/Stress hidden behind "Refine your day".
- **Today's Context**: Hydration (Good/Low), Life Load (Light/Normal/Busy/Overwhelmed), Training Intent (None/Light/Training). Hidden behind "Refine your day". RefineRow component handles semantic colors. Low hydration adds a 5th "Hydrate" action to the daily plan.
