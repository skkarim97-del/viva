...
- **Dashboard (Today tab)**: Status card with feeling chips merged inline (Great/Good/Tired/Stressed), "Today" actions card (Move/Fuel/Hydrate/Recover/Stay Consistent) above coach card, standardized InputRow refine section (Energy/Appetite/Hydration/Protein/Side Effects/Movement. all 4-option rows with uniform navy selected state via InputRow component), coach insight with "Ask your coach anything..." input, end-of-day check-in (3 questions using same InputRow: energy, side effects, protein), and metric tiles (Sleep/Steps/Heart Rate/HRV). Layout order: status card → feedback toast → actions → refine → coach → why plan → check-in → metrics.
