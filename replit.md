...
- **Trends Tab Layout**: Recent Trends summary (top) > Key Insights (3-5 habit+biometric insights) > Correlations (unchanged) > Patterns Detected (unchanged) > Key Metrics at bottom with 3 categories x 3 metrics each: Recovery/Body (Sleep avg, HRV avg, Resting HR avg), Activity (Steps avg, Workouts/week, Active Cal avg), Habits (Weekly %, Streak days, Today completed).
