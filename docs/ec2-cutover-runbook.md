...
- [ ] Login to whoever hosts `viva-ai.com` DNS (see Phase 1).
...
## Phase 1 — Find out where viva-ai.com DNS is hosted

You said you weren't sure. Easiest way to find out: from any computer with
internet, in a terminal, run:

```bash
nslookup -type=NS viva-ai.com
```

(or in PowerShell / cmd: `nslookup -type=NS viva-ai.com`)

The answer will look like one of these:

| Output contains                             | DNS host                |
|---------------------------------------------|-------------------------|
| `awsdns-XX.{com,net,org,co.uk}`             | AWS Route 53            |
| `cloudflare.com` (e.g. `tom.ns.cloudflare.com`) | Cloudflare          |
| `domaincontrol.com`                         | GoDaddy                 |
| `registrar-servers.com`                     | Namecheap               |
| `googledomains.com` / `domains.google`      | Google Domains          |
| something else                              | paste it in chat        |

Once you know, log into that provider's DNS dashboard. **Don't add any
records yet** -- we'll do that in Phase 5.
...
## Phase 8 — Point api.viva-ai.com at the EC2 box

In whatever DNS provider you identified in Phase 1:

1. Add an **A** record:
   - Name: `api`
   - Value: the EC2 instance's Public IPv4 (from Phase 2)
   - TTL: **60 seconds** (so we can roll back fast)
2. Save.
3. Wait 1-2 minutes, then from any computer:
   ```bash
   nslookup api.viva-ai.com
   ```
...
a cert for `api.viva-ai.com` within ~30 seconds. Press Ctrl-C to exit the
follow.
...
```bash
# 1. Healthz
curl -s https://api.viva-ai.com/api/healthz
# -> {"status":"ok"}

# 2. iOS associated-domains JSON (used for universal links)
curl -s https://api.viva-ai.com/.well-known/apple-app-site-association | head

# 3. Android assetlinks JSON
curl -s https://api.viva-ai.com/.well-known/assetlinks.json | head
...
curl -s -H "Authorization: Bearer YOUR_OPERATOR_KEY" \
  https://api.viva-ai.com/api/internal/healthz
...
`VITE_API_BASE_URL=https://api.viva-ai.com/api` on the Replit dashboard
...
block to the Caddyfile for `dashboard.viva-ai.com`. Cleaner for the
...
6. Check Chrome devtools -> Network -> requests are going to
   `api.viva-ai.com`, returning 200, with `Access-Control-Allow-Origin`
...
3. **Critically: keep both URLs serving the same backend during the
   ...
   and the *new* one is hitting `api.viva-ai.com`. Both must work and
...
- Phase 9: output of `curl -I https://api.viva-ai.com/api/healthz`
